...
public class MetallumItems {
    ...
    public static final Map<MetallumMetal, RegistryObject<BucketItem>> METAL_FLUID_BUCKETS = Helpers.mapOfKeys(MetallumMetal.class, metal ->
            register("bucket/metal/" + metal.name(), () -> new BucketItem(MetallumFluids.METALS.get(metal).source(), new Item.Properties().craftRemainder(Items.BUCKET).stacksTo(1))) //todo add to tfc misc stab
    );
